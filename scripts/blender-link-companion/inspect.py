"""Read-only Blender source inspector for the production-link companion.

Run:
    blender --background --factory-startup --python-exit-code 1 \
        <source.blend> --python inspect.py -- --out <manifest.json>

Guarantee (stated exactly as design section 7 requires): this script never saves
over the user's file. Headless Python always mutates in-memory datablocks, so
the honest guarantee is "no save operator is ever called", not "nothing
changes". `assert_no_save_operators()` re-checks that this module contains no
`bpy.ops.wm` save-operator call site at all, which is also why nothing is ever
written next to the source and no `.blend1` sibling can appear.

The emitted manifest carries no absolute path, no source bytes, and no Python
expression. `assert_no_path_leak()` fails the run if any path fragment or the OS
username reaches the manifest.

This is the S0a spike inspector carried forward. Its two Blender traps are
preserved verbatim because both are load-bearing for the published-control
contract: `id_properties_ui(key).as_dict()` fabricates a plus/minus FLT_MAX
range and a 0.0 default for undeclared custom properties, so `rangeDeclared` and
`defaultIsAmbiguous` are the only way a consumer can tell an authored constraint
from an API-invented one.
"""
...
def numeric_property_ui(datablock, key: str) -> dict:
    ...
    Blender's `id_properties_ui(key).as_dict()` always returns
    ...
    trusts those keys verbatim would publish a bogus [-3.4e38, 3.4e38] range and
    a default that contradicts the property's own value. The sentinel comparison
    below is the only way to tell "declared" from "absent".
    """
    try:
