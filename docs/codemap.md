...
| General motion relationships | `entities/scene/model/{motion-relations,motion-relation-commands,scene-camera,scene-camera-commands,node-commands,serialization}.ts`, `features/motion-parenting/{model/authoring,canvas/overlay.tsx}`, `entities/motion/model/presentation.ts`, `widgets/{action-surface,inspector,layers-panel}`, `features/export/model/{bundle,preview,svg,pdf}.ts`, `worker/export-jobs/render/scene-svg.ts` | `motionParent` is camera-independent affine 2D after local motion and before camera projection; exact presentation matrices carry rotated non-uniform-scale shear without persisting skew into authored TRS; editor and Agent share keep-pose bind/detach planning and compound Scene+Motion undo; relations stay structural; clone/component/artboard paths remap internal controller ids; controller paint is editor-only; regenerate `runtime-sampler*.generated.ts` after resolver/presentation dependency changes |
...
cross-store coordinator. **This is the corruption-critical seam** — per AGENTS.md every confirmed
...
breaking change breaks consumers) and the reference-scene freshness gate. The frozen surface list is in
`docs/lean-verification-policy.md`: **`entities/scene` types + store, `entities/motion` types,
...
**Worker / API boundary:** `worker/index.ts` (Hono) owns all `/api/*` and exports the
...
**Verification altitude (from AGENTS.md / `docs/lean-verification-policy.md`):** docs/comment-only
