...
| 2026-09-06 | [Hello Motion](./hello-motion.md) | New `/tutorials` reference scene (Motion category) purpose-authored with no look/mask/mesh-gradient/blend/interactions/grammar/camera capability, reaching the motion-artifact export profile's FLAT sampler tier so its Export → Motion / Code → EMBED runtime lands at 48.2KB gzip instead of the 115-152KB every other gallery scene produces. |
